...
const loadImage = (src: string, errorMessage: string) => new Promise<HTMLImageElement>((resolve, reject) => {
...
function PreviewCanvas({
  ...
  const copy = getLocalizedCopy(useLocale(), "LookGallery")
  ...
  useEffect(() => {
    let cancelled = false
    // eslint-disable-next-line react-hooks/set-state-in-effect -- reset the preview while asynchronous rendering catches up with new inputs.
    setState("loading")

    const render = async () => {
      try {
        const sourceImage = await loadImage(image.src, copy.imageLoadError)
        ...
        if (!workingContext) throw new Error(copy.canvasError)
  ...
  }, [copy.canvasError, copy.imageLoadError, image, large, lut, outputTransform, sourceTransform, targetRef])
  ...
  return (
    <div className="relative aspect-video overflow-hidden rounded-[12px] bg-[color:var(--hp-color-surface)]">
      <canvas
        ...
        aria-label={copy.protectedPreview.replace("{name}", image.name)}
      />
      {state !== "ready" ? (
        <div className="absolute inset-0 grid place-items-center bg-white/40 text-sm text-hp-muted">
          {state === "loading" ? copy.rendering : copy.previewUnavailable}
        </div>
      ) : null}
    </div>
  )
...
const validateFileSize = (file: File, copy: AppMessages["LookGallery"]) => {
  if (file.size > MAX_FILE_SIZE) throw new Error(copy.fileTooLarge.replace("{name}", file.name))
}
...
export function LookGallery() {
  ...
  const [message, setMessage] = useState(copy.preparing)
      ...
      try {
        ...
        const [defaultLutText, storedAssets] = await Promise.all([
          fetch("/look-gallery/default-look.cube").then((response) => {
            if (!response.ok) throw new Error(copy.defaultLutError)
        ...
        if (cancelled) return
        setImages([
          { id: DEFAULT_IMAGE_ID, name: copy.defaultImage, src: "/look-gallery/default-log.jpg", uploaded: false },
          ...restoredImages,
        ])
        setLuts([
          {
        ...
        setMessage(error instanceof Error ? error.message : copy.initializeError)
      }
    }
    void initialize()
    return () => {
  ...
  }, [copy.defaultImage, copy.defaultLutError, copy.initializeError, copy.ready, copy.restored, createObjectUrl])
  ...
  const handleImageUpload = async (files: FileList | null) => {
    ...
    try {
      if (uploadedImageCount + nextFiles.length > MAX_UPLOAD_IMAGES) {
        throw new Error(copy.imageLimit.replace("{count}", String(MAX_UPLOAD_IMAGES)))
      }
      const sessionId = getTabSessionId()
      const additions: GalleryImage[] = []
      for (const file of nextFiles) {
        validateFileSize(file, copy)
        if (!imageExtensionAllowed(file)) throw new Error(copy.unsupportedImage.replace("{name}", file.name))
      ...
      setMessage(copy.imagesAdded.replace("{count}", String(additions.length)))
    } catch (error) {
      setMessage(error instanceof Error ? error.message : copy.imagesAddError)
    }
  }
  ...
  const handleLutUpload = async (files: FileList | null) => {
    ...
    try {
      ...
      for (const file of [...files]) {
        validateFileSize(file, copy)
        if (!file.name.toLowerCase().endsWith(".cube")) throw new Error(copy.invalidCube.replace("{name}", file.name))
      ...
      setMessage(copy.lutsAdded.replace("{count}", String(additions.length)))
    } catch (error) {
      setMessage(error instanceof Error ? error.message : copy.lutsAddError)
    }
  }
  ...
  const removeImage = async (image: GalleryImage) => {
    ...
    setMessage(copy.imageRemoved.replace("{name}", image.name))
  }
    ...
    <section id="look-gallery" className="hp-section-shell scroll-mt-24 md:scroll-mt-28">
      <div className="glass-card p-6 md:p-9 xl:p-10">
        <div className="max-w-3xl">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-hp-muted">{copy.eyebrow}</p>
          <h2 className="hp-heading mt-3 text-2xl font-semibold text-hp md:text-3xl">{copy.title}</h2>
        ...
        <div className="mt-7 grid gap-4 rounded-[12px] border border-white/55 bg-white/35 p-4 md:grid-cols-2 xl:grid-cols-4">
          <label className="grid gap-2 text-sm font-medium text-hp">
            ...
            />
            <span className="text-xs font-normal text-hp-muted">{copy.imageHelp}</span>
          </label>
          <label className="grid gap-2 text-sm font-medium text-hp">
            {copy.addLuts}
            <input
            ...
            />
            <span className="text-xs font-normal text-hp-muted">{copy.lutHelp}</span>
          </label>
        ...
        <details className="mt-4 rounded-[12px] border border-white/55 bg-white/35 p-4">
          <summary className="cursor-pointer text-sm font-semibold text-hp">{copy.settings}</summary>
              ...
              <article key={image.id} className="rounded-[12px] border border-white/55 bg-white/40 p-3 md:p-4">
                ...
                <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-end">
                  <label className="grid min-w-0 flex-1 gap-1 text-xs font-medium text-hp-muted">
                    ...
                    >
                      <option value="batch">{copy.useBatch}</option>
                      {lutOptions.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}
                    </select>
                  </label>
                  {image.uploaded ? (
      ...
      {selectedImage ? (
        <div className="fixed inset-0 z-[80] overflow-y-auto bg-slate-950/60 p-3 md:p-8" role="dialog" aria-modal="true" aria-label={copy.details.replace("{name}", selectedImage.name)} onMouseDown={(event) => {
          ...
          <div className="mx-auto max-w-6xl rounded-[20px] border border-white/55 bg-[color:var(--hp-color-surface)] p-4 shadow-2xl md:p-7">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <p className="mt-2 text-xs leading-5 text-hp-muted">
                  ...
                  LUT: {luts.find((lut) => lut.id === selectedLutId)?.name ?? copy.noLut}<br />
                  LUT target: {localizedLutTargetLabels[lutTarget]}<br />
                  Output: {localizedOutputLabels[outputTransform]}
                </p>
              </div>
              <button className="glass-btn min-h-10 shrink-0 px-4 text-sm font-semibold text-hp" type="button" onClick={() => setSelectedImageId(null)}>{copy.close}</button>
            </div>
            <div className="mt-5">
            ...
            <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              ...
              <button className="glass-btn min-h-11 px-5 text-sm font-semibold text-hp" type="button" onClick={saveProtectedPreview}>{copy.savePreview}</button>
            </div>
          </div>
        </div>
      ) : null}
