...
const validateFileSize = (file: File) => {
  if (file.size > MAX_FILE_SIZE) throw new Error(`${file.name} は50MBを超えています。`)
}
...
export function LookGallery() {
      ...
      try {
        ...
        const [defaultLutText, storedAssets] = await Promise.all([
          fetch("/look-gallery/default-look.cube").then((response) => {
            if (!response.ok) throw new Error("標準LUTを読み込めませんでした。")
        ...
        if (cancelled) return
        setImages([
          { id: DEFAULT_IMAGE_ID, name: "標準Log素材", src: "/look-gallery/default-log.jpg", uploaded: false },
          ...restoredImages,
        ])
        setLuts([
          {
        ...
        setMessage(restoredImages.length || restoredLuts.length ? "このタブの素材を復元しました" : "標準素材で試せます")
      } catch (error) {
        setMessage(error instanceof Error ? error.message : "初期化できませんでした。")
      }
    }
    void initialize()
    return () => {
  ...
  const handleImageUpload = async (files: FileList | null) => {
    ...
    try {
      if (uploadedImageCount + nextFiles.length > MAX_UPLOAD_IMAGES) {
        throw new Error(`追加画像は最大${MAX_UPLOAD_IMAGES}枚です。`)
      }
      const sessionId = getTabSessionId()
      const additions: GalleryImage[] = []
      for (const file of nextFiles) {
        validateFileSize(file)
        if (!imageExtensionAllowed(file)) throw new Error(`${file.name} は対応形式ではありません。`)
      ...
      setMessage(`${additions.length}枚をこのタブへ追加しました`)
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "画像を追加できませんでした。")
    }
  }
  ...
  const handleLutUpload = async (files: FileList | null) => {
    ...
    try {
      ...
      for (const file of [...files]) {
        validateFileSize(file)
        if (!file.name.toLowerCase().endsWith(".cube")) throw new Error(`${file.name} は.cubeではありません。`)
      ...
      setMessage(`${additions.length}件のLUTを追加しました`)
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "LUTを追加できませんでした。")
    }
  }
  ...
  const removeImage = async (image: GalleryImage) => {
    ...
    setMessage(`${image.name}をこのタブから外しました`)
  }
    ...
    <section id="look-gallery" className="hp-section-shell scroll-mt-24 md:scroll-mt-28">
      <div className="glass-card p-6 md:p-9 xl:p-10">
        <div className="max-w-3xl">
          ...
          <h2 className="hp-heading mt-3 text-2xl font-semibold text-hp md:text-3xl">LUTでルックを確かめる</h2>
          <p className="hp-body mt-4 text-base text-hp-muted md:text-lg">
            Log素材と.cubeをブラウザ内で比較できます。追加素材はこのタブだけに保存され、サーバーへ送信されません。
          </p>
        </div>
        ...
        <div className="mt-7 grid gap-4 rounded-[12px] border border-white/55 bg-white/35 p-4 md:grid-cols-2 xl:grid-cols-4">
          <label className="grid gap-2 text-sm font-medium text-hp">
            ...
            />
            <span className="text-xs font-normal text-hp-muted">JPEG、PNG、WebP。各50MB、追加12枚まで</span>
          </label>
          <label className="grid gap-2 text-sm font-medium text-hp">
            LUTを追加
            <input
            ...
            />
            <span className="text-xs font-normal text-hp-muted">.cubeのみ、各50MBまで</span>
          </label>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            ...
            <label className="grid gap-2 text-sm font-medium text-hp">
              ...
              <select className="glass-input min-h-11 px-3 text-sm" value={outputTransform} onChange={(event) => setOutputTransform(event.target.value as OutputTransform)}>
                {Object.entries(outputLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
              </select>
            </label>
          </div>
              ...
              <article key={image.id} className="rounded-[12px] border border-white/55 bg-white/40 p-3 md:p-4">
                ...
                <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-end">
                  <label className="grid min-w-0 flex-1 gap-1 text-xs font-medium text-hp-muted">
                    ...
                    >
                      <option value="batch">一括設定を使う</option>
                      {lutOptions.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}
                    </select>
                  </label>
                  {image.uploaded ? (
      ...
      {selectedImage ? (
        <div className="fixed inset-0 z-[80] overflow-y-auto bg-slate-950/60 p-3 md:p-8" role="dialog" aria-modal="true" aria-label={`${selectedImage.name}の詳細`} onMouseDown={(event) => {
          ...
          <div className="mx-auto max-w-6xl rounded-[20px] border border-white/55 bg-[color:var(--hp-color-surface)] p-4 shadow-2xl md:p-7">
            <div className="flex items-start justify-between gap-4">
              ...
              <button className="glass-btn min-h-10 shrink-0 px-4 text-sm font-semibold text-hp" type="button" onClick={() => setSelectedImageId(null)}>閉じる</button>
            </div>
            <div className="mt-5">
            ...
            <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              ...
              <button className="glass-btn min-h-11 px-5 text-sm font-semibold text-hp" type="button" onClick={saveProtectedPreview}>保護プレビューを保存</button>
            </div>
          </div>
        </div>
      ) : null}
