...
Startup, completed requests, and an unref'ed 30-second timer request replenishment. After a failed creation (for example a spent Notion AI allowance) replenishment waits 30 minutes before trying again.
...
`chatbot-worker@worker.norikane.studio:22` is blocked because that hostname is the
Cloudflare-fronted public endpoint. The working SSH route (direct VPS address) is
recorded in `docs/chatbot-hosted-tier1-heartbeat.md` under "VPS SSH access".
